	...
	slidepic.prototype = {
		...
		_initHtml : function(){
			...
			if(!that.options.ulImg){
				...
				for(i=0; i<that.options.lpos; i++){
					li = $('<li><img src="'+that.options.urls[i]+'" style="width:100%;height:100%;display:block;position: static;top:0px; left:0px;"/></li>');
			...
			if(!that.options.ulBtn && that.options.lpos > 1){
				...
				$(that.div).css({'position' : 'relative','left' : '15%','right' : '0'});
		...
		_initCssAndEvent : function(){
			...
			that.options.ulImg.wrap('<div style="width:100%;height:100%;overflow:hidden;"></div>');
